...
// ProductCard2.tsx - Tailwind CSS Tutorial
            ...
            <div className="max-w-4xl mx-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                    {products.map((product) => (
                        <article className="bg-indigo-100 p-4 rounded-lg" key={product.id}>
                            <img src={product.thumbnailImg} alt={product.productName + " 썸네일"} />
                            <h3 className="font-semibold text-indigo-800">{product.productName}</h3>
                            <p className="text-indigo-600">{product.price.toLocaleString()} 원</p>
                        </article>
                    ))}
                </div>
                {/* 폼 예제 */}
