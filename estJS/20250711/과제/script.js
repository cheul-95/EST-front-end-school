...
function loadMemoStorage() {
    ...
    const saveMemos = localStorage.getItem("memo");
    // 데이터가 있다면 memoData 배열에 저장
    if (saveMemos != null) {
        memoData = JSON.parse(saveMemos);
    }
    // 화면에 메모 목록 출력
    renderMemo();
}
...
function renderMemo() {
    ...
    // 삭제 버튼에 이벤트 리스너 추가
    const h_li = document.createElement("li");
    h_li.innerHTML = `<div class="h_wrap"><div class="h_title">제목</div><div class="h_content">내용</div></div>`;
    memoList.append(h_li);
    memoData.forEach((memo, i) => {
        const li = document.createElement("li");
        li.innerHTML = `<div class="wrap"><div class="title">${memo.title}</div><div class="content">${memo.content}</div><button type="button" class="del_memo" onclick="deleteMemo(${i})">삭제</button>`;
        memoList.append(li);
    });
    return;
}
