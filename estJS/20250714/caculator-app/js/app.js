...
function calculator(operation) {
    try {
        ...
        if (
            !(!isNaN(num1) && typeof num1 === "number") ||
            !(!isNaN(num2) && typeof num2 === "number")
        ) {
            alert("입력값을 확인해주세요");
            return;
        }
        ...
        if (!(!isNaN(result) && typeof result === "number")) {
            throw new Error("결과값을 가져오지 못했습니다.");
        }
        // 결과를 화면에 표시하세요
        resultText.innerHTML = `결과값 : ${result}`;
    } catch (error) {
...
upgradeBtn.addEventListener("click", async () => {
    ...
    userState = changeUserState(userState);
    ...
    if (userState) {
        ...
        resultText.innerHTML = `고급 계산기 사용 권한이 없습니다.`;
    }
});
